...
public class ClustalWDiagnosticsTool
  implements IProteinDiagnosticsTool
{
  ...
  public void initialize() throws InitializationException {
    try {
      ProcessBuilder builder = new ProcessBuilder(new String[] { "clustalw" });
      builder.start();
    } catch (IOException ex) {
      throw new InitializationException(new String[] { "ClustalWDiagnosticsTool: ClustalW is not installed" });
    }
    try {
  ...
  public ProteinDiagnosticResult runDiagnostics(List<ProteinSequence> sequences) {
    ProteinDiagnosticResult result = new ProteinDiagnosticResult();
    NumberFormat formatter = NumberFormat.getPercentInstance();
    ProteinSequence sequence = ProteinSequence.parse(this.sequenceData);
    for (Iterator i$ = sequences.iterator(); i$.hasNext(); ) { ProteinSequence testSequence = (ProteinSequence)i$.next();
      try
      {
        if (testSequence.getName().equals(sequence.getName()))
          testSequence.setHeader(">copy_of_" + testSequence.getHeader().replaceAll(">", ""));

        ClustalWOutput sequenceResult = this.parser.runClustal(sequence, testSequence);
        StringBuffer buffer = new StringBuffer();
        buffer.append("ClustalW Output\n===============================\n\n");
        buffer.append(sequenceResult.getRaw_output());
        buffer.append("\n\nRatios\n====================================\n\n");
        buffer.append("Identity: " + formatter.format(sequenceResult.getIdentity_percentage()) + "\n");
        buffer.append("Strong: " + formatter.format(sequenceResult.getStrong_percentage()) + "\n");
        buffer.append("Weak: " + formatter.format(sequenceResult.getWeak_percentage()) + "\n");
        buffer.append("No Match: " + formatter.format(sequenceResult.getNomatch_percentage()) + "\n");

        result.getTextResults().add(new ProteinDiagnosticTextElement(sequence.getName(), buffer.toString()));
      } catch (Exception ex) {
        throw new RuntimeException("Failed to run diagnostics on sequence:\n" + ex.getMessage());
      }
    }
    return result;
  }
