...
public class HydropathyPlotGenerator
  implements IProteinDiagnosticsTool
{
  ...
  public ProteinDiagnosticResult runDiagnostics(List<ProteinSequence> sequences) {
    ...
    for (Iterator i$ = sequences.iterator(); i$.hasNext(); ) { ProteinSequence sequence = (ProteinSequence)i$.next();
      double[][] data = new double[2][sequence.getLength()];
      for (int i = 0; i < sequence.getLength(); ++i) {
        data[1][i] = HydropathyPlotHelper.averageHydrophobicity(sequence.getData(), i, this.windowSize);
        data[0][i] = i;
      }
      dataset.addSeries(sequence.getName(), data);
    }

    JFreeChart chart = ChartFactory.createXYLineChart(this.plotName, null, null, dataset, PlotOrientation.VERTICAL, true, false, false);
    ...
    plot.getDomainAxis().setTickLabelFont(Font.decode("Arial-PLAIN-15"));

    BufferedImage img = chart.createBufferedImage(800, 600);
    result.getGraphicResults().add(new ProteinDiagnosticImageElement(this.plotName + "(" + this.windowSize + ")", img));
    return result;
