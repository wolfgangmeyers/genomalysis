...
public class ClustalWInterface
{
    ...
    throws IOException
  {
    ...
    File outfile = new File("/tmp/csinput.fsa");
    if (!(outfile.exists()))
    ...
    try
    {
      ...
      ProcessBuilder builder = new ProcessBuilder(new String[] { "clustalw", "-infile=/tmp/csinput.fsa", "-outfile=/tmp/csoutput.aln" });
      ...
      Logger.getLogger(ClustalWInterface.class.getName()).log(Level.SEVERE, null, ex);
    } finally {
      if (fout != null)
        fout.close();
  ...
  private ClustalWOutput parseOutput(List<String> lines, int sequenceLength)
  {
    ...
    for (Iterator i$ = lines.iterator(); i$.hasNext(); ) { line = (String)i$.next();
    ...
    double nomatch_percent = 1D - identity_percent + strong_percent + weak_percent;
    return new ClustalWOutput(raw_output.toString(), identity_percent, strong_percent, weak_percent, nomatch_percent);
  }
}