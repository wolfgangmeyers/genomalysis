...
public class IndexPager {
    ...
    public int getNumberOfPages(){
        if(index == null)
            return 0;
        return (int)Math.ceil(index.getIndexEntries().getEntries().size() / (double)pageSize);
    }
    ...
    public List<IndexEntry> getPage(int pageNr) throws IOException{
        ...
        if(index != null){
            if(pageNr * pageSize < index.getIndexEntries().getEntries().size()){
                ...
                for(int i = startIndex; i < startIndex + pageSize && i < totalEntries; i++){
                    IndexEntry entry = index.getIndexEntries().getEntries().get(i);
                    entries.add(entry);
