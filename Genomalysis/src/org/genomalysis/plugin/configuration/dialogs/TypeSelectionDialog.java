...
public class TypeSelectionDialog extends JDialog
{
  ...
  public Class<?> getSelectedType()
  {
    ...
    if ((index > -1) && (index < this.types.size()))
      result = (Class)this.types.get(index);
  ...
  public boolean showDialog(List<Class<?>> classes)
  {
    ...
    for (Iterator i$ = classes.iterator(); i$.hasNext(); ) { Class clazz = (Class)i$.next();
  ...
  private void initComponents()
  {
    ...
    this.jPanel1.add(this.jComboBox1, "Center");
  ...
  private void jButton1ActionPerformed(ActionEvent evt)
  {
    if (this.jComboBox1.getSelectedIndex() == -1) {
      JOptionPane.showMessageDialog(this.jLabel1, "Select a type", "Error", 0);
    } else {
      this.accept = true;
      setVisible(false);
    }
