...
public class InstancePanel<T> extends JPanel
{
  ...
  private JList lstPluginInstances;
  private JList lstPluginTypes;

  public InstancePanel(final PluginInstanceManager<T> instanceManager, AbstractPluginManager pluginManager, boolean closeable)
  {
    this.observer = null;
    this.dlg = null;
    ...
    final InstancePanel panel = this;
    ...
    this.observer = new IObserver()
    {
      public void update()
      {
        ...
        String[] availablePlugins = instanceManager.getAvailablePlugins();
        String[] arr$ = availablePlugins; int len$ = arr$.length; for (int i$ = 0; i$ < len$; ++i$) { String pluginName = arr$[i$];
          model.add(model.size(), pluginName);
        }
        lstPluginTypes.setModel(model);

        model = new DefaultListModel();
        Iterator instances = instanceManager.getPluginInstances();
        while (instances.hasNext()) {
          PluginInstance instance = (PluginInstance)instances.next();
          model.add(model.size(), instance);
        }
        lstPluginInstances.setModel(model);
      }
  ...
  public InstancePanel(PluginInstanceManager<T> instanceManager, AbstractPluginManager pluginManager)
  {
  ...
  private void initComponents()
  {
    ...
    this.lstPluginInstances = new JList();
    ...
    this.lstPluginTypes.addListSelectionListener(new ListSelectionListener() {
    ...
    this.btnRenameInstance.setToolTipText("<html>\nRenames the selected filter in<br>\nthe <font color=\"00AA00\">Instances</font> list\n</html>");
    ...
    this.lstPluginInstances.addListSelectionListener(new ListSelectionListener() {
      public void valueChanged(ListSelectionEvent evt) {
        lstPluginInstancesValueChanged(evt);
      }

    });
    this.lstPluginInstances.addMouseListener(new java.awt.event.MouseAdapter() {
      public void mouseClicked(MouseEvent evt) {
  ...
  private void btnCreateInstanceActionPerformed(ActionEvent evt) {
    ...
    if (index > -1)
      try {
        ...
        Logger.getLogger(InstancePanel.class.getName()).log(Level.SEVERE, null, ex);
  ...
  {
    PluginInstance instance = (PluginInstance)this.lstPluginInstances.getSelectedValue();
    if (instance != null)
  ...
  {
    PluginInstance instanceWrapper = (PluginInstance)this.lstPluginInstances.getSelectedValue();
    if (instanceWrapper != null)
      try {
        Object instance = instanceWrapper.getPluginInstance();
        IPropertyConfigurator configurator = this.pluginManager.getConfigurator(instance);
  ...
  private void btnInfoActionPerformed(ActionEvent evt)
  {
    ...
    if (this.lstPluginTypes.getSelectedIndex() != -1) {
      Class clazz = this.instanceManager.getPluginClass(this.lstPluginTypes.getSelectedIndex());
      documentation = ConfigurationTables.getDocumentationTable().getDocumentation(clazz);
  ...
  private void lstPluginTypesMouseClicked(MouseEvent evt) {
    if ((this.lstPluginTypes.getSelectedIndex() > -1) && (evt.getClickCount() == 2))
      btnCreateInstanceActionPerformed(null);
  }

  private void lstPluginInstancesMouseClicked(MouseEvent evt)
  {
    if ((this.lstPluginInstances.getSelectedIndex() > -1) && (evt.getClickCount() == 2))
