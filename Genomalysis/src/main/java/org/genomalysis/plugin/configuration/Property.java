...
public class Property {
    ...
    public void configure(JComponent base) throws ConfigurationException {
        try {
            Object currentValue = getGetter().invoke(targetObject,
                    new Object[] {});
            if (currentValue == null) {
                try {
                    propertyType.asSubclass(Number.class);
                    Constructor<?> constructor = propertyType
            ...
            // System.out.println("Property.configure: newValue is " + newValue
            // == null ? "null" : newValue.toString());
            getSetter().invoke(targetObject, newValue);
        } catch (Exception ex) {
            ex.printStackTrace();
            throw new ConfigurationException(
                    "There was a problem configuring this property:\n"
                            + ex.getMessage());
        }
    }
