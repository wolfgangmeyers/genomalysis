...
public final class ConfigurationTables {
    ...
    public static void introspectClass(Class<?> clazz) {
        if ((clazz.getAnnotation(Documentation.class) != null)
                || (clazz.getAnnotation(Author.class) != null)) {
            documentationTable.registerDocumentation(clazz);
        }

        Configurator configuratorAnnotation = (Configurator) clazz
                .getAnnotation(Configurator.class);

        if (configuratorAnnotation != null) {
            Class<?> configuratorClass = configuratorAnnotation.value();
            try {
                Constructor<?> cfgCtor = configuratorClass
                        .getConstructor(new Class[0]);

                IPropertyConfigurator configurator = (IPropertyConfigurator) cfgCtor
                        .newInstance(new Object[0]);
                propertyConfiguratorTable.RegisterPropertyConfigurationDialog(
                        clazz, configurator);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        ...
        ReplacesType replacementAnnotation = (ReplacesType) clazz
                .getAnnotation(ReplacesType.class);
        if (replacementAnnotation != null)
            try {
                ...
                Logger.getLogger(ConfigurationTables.class.getName()).log(
