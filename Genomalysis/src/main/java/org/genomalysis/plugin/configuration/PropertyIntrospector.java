...
public class PropertyIntrospector {
    ...
    public static PropertyDescriptor getProperties(Object obj)
        ...
        for (String propertyName : propertyNames) {
            ...
            // now, let's see if we can find an IPropertyConfigurationDialog on
            // the
            ...
            if (getter.getReturnType() == List.class) {
                ...
                if (returnType instanceof ParameterizedType) {
                    ...
                    if ((args.length > 0) && (args[0] instanceof Class)) {
                        ...
                        GenericWrapper listWrapper = new GenericWrapper(
                                genericListType, innerValue);
                        listWrapper.setDescription("List");
                        GenericWrapper outerWrapper = new GenericWrapper(
                                List.class, listWrapper);
                        try {
                            getter = GenericWrapper.class.getMethod("getValue",
                                    new Class[0]);
                            setter = GenericWrapper.class.getMethod("setValue",
    ...
    private static void setPropertyConfigurator(Property property, Object host,
            Class<?> type)
            throws org.genomalysis.plugin.configuration.ConfigurationException {
        Method setter = property.getSetter();

        PropertySetter setterAnnotation = (PropertySetter) setter
        ...
        if (setterAnnotation.ConfiguratorType() == VoidConfigurator.class) {
            ...
            if (ConfigurationTables.getPropertyConfiguratorTable()
                    .IsClassRegisteredForConfiguration(propertyType)) {
                IPropertyConfigurator dialog = ConfigurationTables
                        .getPropertyConfiguratorTable().getConfigurationDialog(
                                propertyType);
                property.setConfigurator(dialog);
            } else if (propertyType.getAnnotation(Configurator.class) != null) {
                Configurator propertyTypeAnnotation = (Configurator) propertyType
                        .getAnnotation(Configurator.class);
                Class<? extends IPropertyConfigurator> configClass = propertyTypeAnnotation
                        .value();
                IPropertyConfigurator configurator = getConfigurationDialogInstance(
                        configClass, propertyType, property.getPropertyName());
                property.setConfigurator(configurator);
            } else {
                property.setConfigurator(VoidConfigurator.getInstance());
            }
        } else {
            Class<? extends IPropertyConfigurator> configClass = setterAnnotation
                    .ConfiguratorType();
            property.setConfigurator(getConfigurationDialogInstance(
                    configClass, host.getClass(), property.getPropertyName()));
        }
    }

    public static void setPropertyConfigurator(Property property, Object host)
            throws org.genomalysis.plugin.configuration.ConfigurationException {
        Method setter = property.getSetter();
        ...
        if (setterAnnotation.ConfiguratorType() == VoidConfigurator.class) {
            ...
            if (currentPropertyValue == null) {
                ...
                setPropertyConfigurator(property, host,
                        currentPropertyValue.getClass());
    ...
    private static IPropertyConfigurator getConfigurationDialogInstance(
            Class<? extends IPropertyConfigurator> configuratorClass,
            Class<?> configuringClass, String propertyName)
            throws ConfigurationException {
        try {
            Constructor<? extends IPropertyConfigurator> constructor = configuratorClass
                    .getConstructor();
            IPropertyConfigurator instance = constructor.newInstance();
            return instance;
        } catch (NoSuchMethodException ex) {
            throw new ConfigurationException(
                    "Property config dialog defined for property "
                            + propertyName
                            + " has no default constructor defined");
        } catch (IllegalAccessException ex) {
            throw new ConfigurationException(
                    "There is a default constructor for configuration dialog defined for property "
                            + propertyName + " in class"
                            + configuringClass.getName()
                            + ", but it is not public");
        } catch (Exception ex) {
            throw new ConfigurationException(
                    "There was an unexpected problem creating configuration dialog for property "
                            + propertyName + " in class " + configuringClass);
        }
    }

    private static void validatePropertyMethods(String propertyName,
            Method getter, Method setter) throws ConfigurationException {
        // make sure setter takes one argument and returns void
        if (!(setter.getReturnType() == void.class && setter
                .getParameterTypes().length == 1)) {
            throw new ConfigurationException(
                    "Setter method for property "
                            + propertyName
                            + " does not match signature for setter methods (void setMethodName(Object value))");
        }
        // make sure getter returns an object of the same type that setter
        // takes,
        // and make sure getter takes no arguments
        if (getter.getReturnType() != setter.getParameterTypes()[0]) {
            throw new ConfigurationException("Getter and Setter for property "
                    + propertyName + " must be of the same type");
        }
        if (getter.getParameterTypes().length > 0) {
            throw new ConfigurationException("Getter method for property "
                    + propertyName + " cannot accept parameters");
        }
    }

    private static void findOrphanedSetters(Class<?> clazz,
            List<String> propertyNames) throws ConfigurationException {

        for (Method method : clazz.getDeclaredMethods()) {
            if (method.getAnnotation(PropertySetter.class) != null) {
                PropertySetter setter = method
                        .getAnnotation(PropertySetter.class);
                if (!propertyNames.contains(setter.PropertyName())) {
                    throw new ConfigurationException("Property with name "
                            + setter.PropertyName()
                            + " has setter but no getter");
                }
            }
        }
    }
    ...
    private static Method findPropertyMethod(Class<?> clazz,
            String propertyName, Class<? extends Annotation> annotationType)
            throws ConfigurationException {
        for (Method method : clazz.getDeclaredMethods()) {
        ...
        throw new ConfigurationException(annotationType.getName()
                + " with name " + propertyName
                + " could not be found in class " + clazz.getName());
    }

    private static boolean annotationMatchesProperty(Annotation ann,
            String propertyName) {
    ...
    private static List<String> getPropertyNames(Class<?> clazz)
            throws ConfigurationException {
