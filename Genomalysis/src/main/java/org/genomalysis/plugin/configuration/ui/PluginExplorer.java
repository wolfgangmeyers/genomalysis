...
public class PluginExplorer extends JFrame {
    ...
    private JList<String> lstFoundTypes;
    ...
    private void btnPluginTypeInfoActionPerformed(ActionEvent evt) {
        ...
        if (index == -1) {
            System.out
                    .println("WARNING: btnPluginTypeInfo should be disabled, but it was clicked anyway!");
            this.btnPluginTypeInfo.setEnabled(false);
        } else {
            Class<?> clazz = this.foundTypes.get(index);
            String documentation = ConfigurationTables.getDocumentationTable()
    ...
    private void btnAddPluginTypeActionPerformed(ActionEvent evt) {
        ...
        if (index == -1) {
            ...
            Class<?> clazz = this.foundTypes.get(index);
            PluginInstanceManager<?> instanceManager = new PluginInstanceManager<>(
                    clazz, this.pluginManager);
            InstancePanel<?> panel = new InstancePanel<>(instanceManager,
