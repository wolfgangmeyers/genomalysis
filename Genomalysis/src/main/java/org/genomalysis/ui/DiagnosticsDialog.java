...
public class DiagnosticsDialog extends JDialog {
    ...
    public void showDialog(ProteinDiagnosticResult resultToDisplay) {
        ...
        for (ProteinDiagnosticTextElement text : resultToDisplay
                .getTextResults()) {
            DiagnosticsTextDisplay display = new DiagnosticsTextDisplay(
                    text.getText());
            this.resultsPanel.addTab(text.getName(), display);
        }
        for (ProteinDiagnosticImageElement img : resultToDisplay
                .getGraphicResults()) {
            DiagnosticsImageDisplay display = new DiagnosticsImageDisplay(
                    img.getImage());
