...
public class FileIndexIO {
    ...
    public void writeBinaryIndex(Index index, File outputFile)
            throws IOException {
    ...
    public Index readBinaryIndex(File inputFile) throws IOException {
        ...
        try {
            ...
            Logger.getLogger(FileIndexIO.class.getName()).log(Level.SEVERE,
                    null, ex);
    ...
    public Index indexFasta(File inputFile) throws IOException {
        /**
         * Start at beginning set main index to 0 set file length
         * 
         * get buffer of 1000000 characters (or the last amount of characters)
         * find every instance of ">" in characters for each instance, get name
         * of sequence (shortened) and start and end index index of last ">" and
         * next ">" if it is the last sequence, test for end of file if end of
         * file, this is the last sequence in the file: add it otherwise, repeat
         * until end of file if this is not the last sequence, add it to the
         * list
         */
        ...
        try {
            ...
            while (!eof) {
                ...
                int last = 0;
                // read the next chunk
                int bytesRead = rm.read(buffer, 0,
                        (int) Math.min(length - index, buffer.length));
                // the buffer should fill up, unless we've reached the end of
                // the
                // file
